...
methods = ["Dinamico", "Memoizacao", "Recursivo"]
filenames = ["dinamic_algo.csv", "memo_algo.csv", "recursive_algo.csv"]

def read_data(filename, m_range):
    n_size, values = [], []
    with open(os.path.join(filename), "r") as f:
        reader = csv.reader(f)
        data = np.array(list(reader))
        n_size = np.array([int(x[0]) for x in data[::20]])
        for i in range(0, m_range): # Para insertion sort, eh necessario alterar este 23 por 13
...
# Array of arrays
size_n_rad, vals_n_rad = read_data(filenames[0], 10+1)
size_n_mer, vals_n_mer = read_data(filenames[1], 6+1)
size_n_ins, vals_n_ins = read_data(filenames[2], 3+1)

print(len(vals_n_mer))
...
for i, n in enumerate(size_n_rad): # Except insertion, all methods have all n's
    if n>640:
        break
    t_test_diffs(vals_n_rad[i], vals_n_mer[i], methods[0], methods[1], n, csv_writer)
    if n <= 20:
        methods[0], methods[2], n, t_test_diffs(vals_n_rad[i], vals_n_ins[i], methods[0], methods[2], n, csv_writer)
        methods[1], methods[2], n, t_test_diffs(vals_n_mer[i], vals_n_ins[i], methods[1], methods[2], n, csv_writer)
